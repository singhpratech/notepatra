...
…regardless of what was actually downloaded. Since v0.1.18 we've shipped
`.tar.gz` tarballs (no AppImage), so the dialog has been silently giving
the wrong instructions for ~60 releases. Tarball users were being told to
drop an AppImage that didn't exist.
...
> binary (the one being upgraded *from*), so v0.1.80 → v0.1.81 still
> shows the bad text. The fix kicks in from v0.1.81 → v0.1.82 onward.
