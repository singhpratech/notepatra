...
AIPanel::AIPanel(QWidget *parent) : QWidget(parent) {
    ...
    // hardening: mode-toggle mid-stream cancels the in-flight request so the
    // response doesn't keep streaming into the now-rebuilt chat surface
    ...
    connect(m_chatMode,   &QAbstractButton::toggled, this, [applyModeWithCancel](bool on) { if (on) applyModeWithCancel(); });
    connect(m_codingMode, &QAbstractButton::toggled, this, [applyModeWithCancel](bool on) { if (on) applyModeWithCancel(); });
    connect(m_dataMode,   &QAbstractButton::toggled, this, [applyModeWithCancel](bool on) { if (on) applyModeWithCancel(); });
