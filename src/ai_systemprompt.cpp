    ...
    switch (intent) {
        ...
        case Intent::DataAnalyst:
            ...
            return QStringLiteral(
                ...
                "Supported `type` values (v0.1.76): `line`, `area`, `bar`, "
                "`horizontal-bar`, `grouped-bar`, `stacked-bar`, "
                "`stacked-horizontal-bar`, `pie`, `donut`, `scatter`, "
                "`histogram`, `boxplot`.\n\n"
                "For Vega-Lite specs (heatmaps, geo-shapes, complex layered "
                "specs not covered above), use the `generate_chart` tool with "
                "a full v5 spec instead of the fenced block.\n\n"
                "── PROSE STRUCTURE ──────────────────────────────────────────\n"
                "Structure answers as: brief Findings (1–3 bullets) → Method "
                "(one sentence on the query you ran) → Suggested follow-ups "
                "(1–2 questions the user could ask next). For database "
                "mutations (INSERT/UPDATE/DELETE/DDL), do NOT run them "
                "silently — describe what you would run, and ask the user to "
                "confirm. SELECT queries run freely.");
    }
