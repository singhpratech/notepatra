...
- **Result**: the speed of C++, the safety of Rust. The bare executable is **under 10 MB** on every platform (~9.7 MB Linux x64, similar on macOS / Windows). Latest v0.1.87 download sizes: **3.5 MB** Linux x64 tar.gz · **3.3 MB** Linux ARM64 tar.gz · **26.5 MB** macOS DMG (with bundled Qt) · **40.6 MB** Windows MSI · **32.2 MB** Windows NSIS · **36.7 MB** Windows portable zip. _Installed footprint on Windows is ~75-85 MB after the MSI extracts bundled Qt + QScintilla DLLs — normal for any Qt-based installer._
...
### Or download manually — [Latest release: v0.1.87](https://github.com/singhpratech/notepatra/releases/latest)
...
| 🍎 **macOS Apple Silicon** (M1–M4) | [`.dmg`](https://github.com/singhpratech/notepatra/releases/latest) | **26.5 MB** | `Notepatra.app` with Qt frameworks bundled. Drag to Applications. |
...
|---|---|---|
| 🪟 **Windows** | [`notepatra-x.x.x.msi`](https://github.com/singhpratech/notepatra/releases/latest) | `msiexec /i notepatra-0.1.87.msi /quiet` — installs to `C:\Program Files\Notepatra\`, adds system PATH, registers HKCR file associations, all-users Start Menu. WiX-built, MajorUpgrade-aware, SCCM-friendly. |
...
| 🐧 **Debian / Ubuntu / Mint / Pop!_OS** (x64 + ARM64) | [`notepatra_0.1.87_amd64.deb`](https://github.com/singhpratech/notepatra/releases/latest) | `sudo apt install ./notepatra_0.1.87_amd64.deb` — installs to `/opt/notepatra/` + symlink at `/usr/bin/notepatra`, hicolor icons, `.desktop` registration. ARM64: replace `amd64` → `arm64`. |
| 🐧 **Fedora / RHEL / CentOS Stream / Rocky / Alma** (x64 + ARM64) | [`notepatra-0.1.87-1.x86_64.rpm`](https://github.com/singhpratech/notepatra/releases/latest) | `sudo dnf install ./notepatra-0.1.87-1.x86_64.rpm` — same layout as the .deb. ARM64: replace `x86_64` → `aarch64`. Bundles QScintilla 2.14.1 alongside the binary because Fedora ships an incompatible packaging. |
| 🐧 **Arch / openSUSE Tumbleweed / Manjaro / EndeavourOS / other glibc 2.38+** | [`Notepatra-0.1.87-x86_64.AppImage`](https://github.com/singhpratech/notepatra/releases/latest) | `chmod +x Notepatra-0.1.87-x86_64.AppImage && sudo cp Notepatra-0.1.87-x86_64.AppImage /opt/notepatra.AppImage && sudo ln -s /opt/notepatra.AppImage /usr/local/bin/notepatra`. Requires glibc 2.38+ (Ubuntu 24.04+, Fedora 40+, Arch, Tumbleweed). Older distros: use the .deb / .rpm. |
...
| OS | Artefact | Silent admin install |
|---|---|---|
| 🪟 **Windows** | [`notepatra-local-ai-0.1.87.msi`](https://github.com/singhpratech/notepatra/releases/latest) | `msiexec /i notepatra-local-ai-0.1.87.msi /quiet` — installs to `C:\Program Files\Notepatra Local AI\`, distinct UpgradeCode so SCCM treats it as its own product. Add/Remove Programs shows "Notepatra Local AI". |
| 🐧 **Debian/Ubuntu x64** | [`notepatra-local-ai_0.1.87_amd64.deb`](https://github.com/singhpratech/notepatra/releases/latest) | `sudo apt install ./notepatra-local-ai_0.1.87_amd64.deb` |
| 🐧 **Debian/Ubuntu ARM64** | [`notepatra-local-ai_0.1.87_arm64.deb`](https://github.com/singhpratech/notepatra/releases/latest) | `sudo apt install ./notepatra-local-ai_0.1.87_arm64.deb` |
...
On Linux the two flavors share the same `notepatra` binary name on disk; `apt` Conflicts ensures only one of `notepatra` / `notepatra-local-ai` is installed at a time, swap transactionally with `sudo apt install ./notepatra-local-ai_0.1.87_amd64.deb`. On Windows the two MSIs are independent products (different UpgradeCode + ProductName + install dir) so they can coexist if needed; admins typically push one or the other based on policy. `notepatra --version` self-identifies the build: `Notepatra v0.1.87 (cloud-free / local-ai)` for the local-ai flavor, `Notepatra v0.1.87` for the regular flavor.
...
|---|---|---|
| [**v0.1.87**](https://github.com/singhpratech/notepatra/releases/tag/v0.1.87) | 2026-05-15 | **Save As file-type dropdown + large-file load speed-up.** Two user-reported pain points fixed: dead Save As dropdown vs Notepad++ (`src/mainwindow.cpp:1655` was passing only `"All Files (*)"` so the bottom-of-dialog dropdown had one entry), and 100+ MB files slower than the "up to 2 GB" promise. New `buildSaveAsFilters()` helper in `src/lexerutils.{h,cpp}` returns ~72 language entries matching Notepad++ behaviour (Plain Text · Markdown · Python · JS/TS · C/C++ · Rust · Go · SQL · JSON · YAML · TOML · CSV · HTML · CSS · …); pre-selects the filter matching the current tab's language. Same fix in `closeTab()` "Save before close?" prompt which had no filter at all. **Large-file load — 3 fixes that compound:** (1) UTF-8 fast path in `rust-core/src/file_io.rs` — skip `UTF_8.decode()` when mmap bytes are already valid UTF-8 (was allocating a fresh 118 MB String even when not needed; saves 118 MB heap + 250 ms on 118 MB file). (2) EOL detection bounded to first 64 KB — was scanning 118 MB twice for ~300 ms. (3) Drop `CString` round-trip in Rust→C FFI — `FileLoadResult.text` now allocates a `Box<[u8]>` instead of a CString (skips O(N) NUL scan + extra 118 MB allocation + appended NUL byte). New `npc_free_file_text(ptr, len)` reclaims the boxed slice by length. **Large-file UX gates**: files > 50 MB also disable word wrap + indent guides + edge column (word-wrap recalc was stuttering the typing cursor); Find → Mark All caps to 10000 highlights with "Marked first 10000 of N — capped for performance" status. Net for 118 MB UTF-8 file: peak RAM 700 MB → ~360 MB; load 1.5 s → ~600 ms. Chunked SCI_APPENDTEXT load deferred to v0.1.88. 31/31 ctest + 130/130 cargo test pass. |
