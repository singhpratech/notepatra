...
### Or download manually — [Latest release: v0.1.56](https://github.com/singhpratech/notepatra/releases/latest)
...
|---|---|---|
| [**v0.1.56**](https://github.com/singhpratech/notepatra/releases/tag/v0.1.56) | 2026-05-09 | **Windows mojibake hotfix + chat input auto-grow + llama.cpp dropdown clarity.** **Critical Windows fix**: every emoji in the UI was rendering as garbage (`📊` → `ðŸ"š`, `🔒` → `ðŸ""`, `…` → `â€¦`, `≈` → `â‰ˆ`, `·` → `Â·`) because MSVC reads C++ source as the system code page (Windows-1252) by default rather than UTF-8. Added `/utf-8` compile flag in CMakeLists; affects every emoji and accented character in the codebase. Linux + macOS were already fine. **Chat input auto-grow**: the bottom message box now expands as you type multi-line content (up to 140 px), driven by the document's actual rendered height — single typed lines that visually wrap also trigger the grow, not just Enter-separated paragraphs. Auto-collapses to 40 px after Send. **llama.cpp dropdown clarity**: when llama-server isn't running and no GGUF is loaded, the curated 12-model catalog now reads as suggestions to download — disabled header item `— llama-server not running · pick one to download —` at the top, every catalog row prefixed with `↓ `, status banner already explains the next step. Bug-fix-only release, no schema changes. |
