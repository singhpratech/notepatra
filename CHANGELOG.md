...
- **Every control explains itself on hover.** Both mode radios, the length slider and spin box, each character-set checkbox, the look-alike and one-of-each switches, the word count, separator, capitalise and append-digits options, the readout, the strength meter, the Generate button and both group boxes carry a tooltip saying what the control does and what it costs — including the ones that add no entropy at all (a fixed separator and blanket capitalisation are not random choices, and the tooltips say so).

### Fixed
- **Spin-box and combo arrows rendered as solid black squares.** The panel stylesheet used the CSS border-triangle idiom — `width: 0` plus transparent side borders — which a browser renders as a triangle and Qt's stylesheet engine renders as a filled box. It would not have looked native on Windows or macOS either. `QSpinBox` and `QComboBox` are now left out of the stylesheet entirely and themed by `QPalette`, so the platform style keeps drawing its own arrows and the disabled state still greys correctly when a value sits at its minimum. This matches how `preferences.cpp` and `notes.cpp` already theme their widgets. Note `sqlfmtpanel.cpp` and `restclient.cpp` still style these classes in QSS without sub-control rules and have the same defect; that is a separate fix.
...
- **`clippy::chunks_exact_to_as_chunks` (new in Rust 1.98) failed the Rust Quality job** against pre-existing code — two UTF-32 decoders in `rust-core` and the minimal SHA-1 in the sidecar. Rewritten with `as_chunks`, which yields the same full chunks and drops the same remainder. Because that hash derives the socket name, it now carries a known-answer test against all four FIPS 180-1 vectors, including the million-byte multi-block case.

### Internal
- 79 C++ suites on the Full build (76 on Lite), up from 76 and 73. The three new suites are **`test_passwordgen`** (the entropy maths and the CSPRNG draw paths), **`test_passwordgen_panel`** (the widgets) and **`test_qss_arrows`** (a source lint that fails the build if any stylesheet deletes a spin box's or combo box's arrows). Full offscreen `ctest` is 79/79 green. The two new suites are **`test_passwordgen`** (the entropy maths and the CSPRNG draw paths) and **`test_passwordgen_panel`** (the widgets). Full offscreen `ctest` is 78/78 green.
...
- **A true Full flavor for macOS and Windows.** Previously only Linux published a `-full` build; the mac DMG / Windows MSI shipped without WebEngine despite the docs saying otherwise (the v0.1.106 Windows zip had zero WebEngine DLLs). Every platform now offers a Lite (bare) and a Full download — Full bundles DuckDB everywhere, plus the QtWebEngine inline Vega-Lite chart renderer on Linux/Windows (macOS Full is DuckDB-only; the native fenced ```chart QtCharts renderer works on every platform).
- **The app now names its build flavor.** `notepatra --version`, the window title bar, and the About dialog identify the running build by name — **Notepatra Lite** / **Notepatra Full** (Full = the bundled DuckDB engine), and **Notepatra Local AI Lite** / **Notepatra Local AI Full** for the cloud-free builds — so you can tell at a glance which binary you have.
